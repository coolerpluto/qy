...
public interface OracleSQL {
    ...
    String QUERY_CONSTRAINT_SQL = "SELECT uc.CONSTRAINT_NAME ,uc.CONSTRAINT_TYPE, ucc.COLUMN_NAME,uc.TABLE_NAME\n" +
            "FROM user_tables ut\n" +
            "JOIN USER_CONSTRAINTS uc ON ut.TABLE_NAME = uc.TABLE_NAME \n" +
            "JOIN USER_CONS_COLUMNS ucc ON uc.CONSTRAINT_NAME = ucc.CONSTRAINT_NAME ";

    //获取索引名称、索引所加字段名称、索引类型、索引所在表名
    String QUERY_INDEX_SQL = "SELECT ui.INDEX_NAME  , uic.COLUMN_NAME ,uc.CONSTRAINT_TYPE, ui.TABLE_NAME\n" +
            "FROM user_tables ut\n" +
